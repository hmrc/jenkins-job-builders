// This is a copy of https://github.com/jenkinsci/job-dsl-plugin/blob/master/job-dsl-core/src/main/groovy/javaposse/jobdsl/dsl/helpers/publisher/PublisherContext.groovy
// with the "keepLongStdio(junitContext.retainLongStdout)" line removed from inside archiveJunit, because keepLongStdio has been removed from https://plugins.jenkins.io/junit/
// Having this file here causes it to be used as a local patch, since https://plugins.jenkins.io/job-dsl/ is unmaintained.

// When bumping jobDslVersion in build.gradle, check whether this file has changed:
// https://github.com/jenkinsci/job-dsl-plugin/commits/master/job-dsl-core/src/main/groovy/javaposse/jobdsl/dsl/helpers/publisher/PublisherContext.groovy
...
class PublisherContext extends AbstractExtensibleContext {
    ...
    void archiveJunit(String glob, @DslContext(ArchiveJUnitContext) Closure junitClosure = null) {
        ...
        publisherNodes << new NodeBuilder().'hudson.tasks.junit.JUnitResultArchiver' {
            testResults(glob)
            // keepLongStdio has been removed, as seen in:
            // https://jenkinsci.github.io/job-dsl-plugin/#method/javaposse.jobdsl.dsl.helpers.publisher.PublisherContext.archiveJunit
            // keepLongStdio(junitContext.retainLongStdout)
            // Am not using retainLongStdout() because it would keep all output even when a test succeeds
            // https://github.com/jenkinsci/junit-plugin/blob/master/src/main/resources/hudson/tasks/junit/JUnitResultArchiver/help-stdioRetention.html
