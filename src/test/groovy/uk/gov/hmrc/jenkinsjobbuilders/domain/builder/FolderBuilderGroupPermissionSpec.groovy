...
class FolderBuilderGroupPermissionSpec extends AbstractJobSpec {
    ...
    def 'it correctly configures a fully populated example'() {
        ...
        final FolderBuilder folderBuilder = new FolderBuilder(folderName)
            ...
            .withGroupPermissions(permissions)
        ...
        with(folder.node) {
            ...
            permissions.each { permission ->
                configuredPermissions.tail().find { configuredPermission ->
                    ...
                    "GROUP:${permission.permission}:${permission.ldapIdentifier}" == configuredPermission.text()
                }
            }
        }
    }
    ...
    def 'it should throw an exception when it cannot map a permission, identifiying the permission that cannot be used'() {
        given:
        final String folderName = "my-first-folder"
        final Set<GroupPermission> permissions = [permissionSetting("ci-admin", "hudson.model.Item.MadeUpPermission")]
        final FolderBuilder folderBuilder = new FolderBuilder(folderName).withGroupPermissions(permissions)
        ...
        e.message.contains("Could not assign group permission: hudson.model.Item.MadeUpPermission to LDAP identifier: ci-admin")
